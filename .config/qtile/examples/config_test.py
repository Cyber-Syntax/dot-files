...
if not terminal:
    terminal = guess(["alacritty", "kgx", "kconsole", "xterm", "urxvt", "kitty", "st"])

if not browser:
    browser = guess(
        ["firefox", "chrome", "chromium", "librewolf", "vivaldi", "waterfox", "brave"]
    )

if not file_manager:
    file_manager = guess(["thunar", "pcmanfm", "nautilus", "dolphin"])
...
layouts = [
    getattr(layout, i)(**(layout_theme | layouts_tweaks.get(i, {}))) for i in layouts
]
...
class Wallpaper:
    ...
    def getSavedWallpaper():
        with open(expanduser("~/.config/nitrogen/bg-saved.cfg"), "r") as file:
            path = (
                file.read().splitlines()[1].removeprefix("file=").strip()
            )  # Get saved background path
            directory = normpath(path[::-1].split("/", 1)[1][::-1])
            name = path.split("/")[-1]

        if (
            normpath(directory) == normpath(wallpapers_path)
            and name in Wallpaper.wallpapers
        ):  # Checks if the background folder is wallpapers_path is in wallpapers
            return Wallpaper.wallpapers.index(
                name
            )  # Set the pointer on the saved background
    ...
    def init():
        ...
        Wallpaper.wallpapers.sort(
            key=lambda w: getctime(f"{wallpapers_path}{w}")
    ...
    def set():
        system(
            f"nitrogen --save --set-{Wallpaper.mode} {wallpapers_path}{Wallpaper.formatName(Wallpaper.wallpapers[Wallpaper.current])}"
        )
...
keys = [
    ...
    *[
        Key(
            [mod, "control"],
            num_keys[index],
            lazy.group.setlayout(layout.name),
            desc=f"Switch to the {layout.name} layout",
        )
    ...
    *[
        Key(
            ...
            desc=f"Switch to the {group.name} group",
        )
    ...
    *[
        Key(
            ...
            desc=f"Move focused window to the {group.name} group",
        )
        for index, group in enumerate(groups)
    ],
]
...
left = [
    ...
    widget.TextBox(
        "Wallpaper",
        padding=0,
        mouse_callbacks={"Button4": Wallpaper.next(), "Button5": Wallpaper.previous()},
    ),
    widget.TextBox(
        "\uf061",
        mouse_callbacks={
...
right = [
    ...
    widget.CurrentLayout(
        fmt=currentLayout,
        mouse_callbacks={"Button2": lambda: None, "Button3": lazy.prev_layout()},
    ),
    space,
    widget.TextBox(
        "⏻",
